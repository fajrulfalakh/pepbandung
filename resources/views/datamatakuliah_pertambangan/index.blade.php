...
<head>
  ...
  <!--end::Base Path -->
  <meta charset="utf-8" />
  <title>PEP BANDUNG | Matakuliah Pertambangan</title>
  ...
  <div class="kt-grid__item kt-grid__item--fluid kt-grid kt-grid--hor kt-wrapper" id="kt_wrapper">
    ...
    <div class="kt-subheader   kt-grid__item" id="kt_subheader">
      <div class="kt-subheader__main" style="justify-content: flex-end;">
        <h3 class="kt-subheader__title"> Matakuliah Pertambangan </h3>
        <span class="kt-subheader__separator kt-hidden"></span>
        <div class="kt-subheader__breadcrumbs">
          <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#addMatakuliah_pertambangan">Tambah Data
          </button>
    ...
    <div class="kt-content  kt-grid__item kt-grid__item--fluid" id="kt_content">
      <div class="row">
            ...
            <div class="portlet-body table-scrollable">
              <table class="table table-striped table-hover table-bordered jquery-datatables">
                ...
                <tbody>
                  ...
                  @foreach ($dataMatakuliah_Pertambangan as $m)
                  <tr>
                    <td>{{ $m->id_matakuliah_pertambangan }}</td>
                    <td>{{ $m->kd_matakuliah_pertambangan }}</td>
                    <td>{{ $m->namamatakuliah_pertambangan }}</td>
                    <td>{{ $m->semester}}</td>
                    <td>{{ $m->teori}}</td>
                    <td>{{ $m->praktek}}</td>
                    <td>{{ $m->kerjalapang}}</td>
                    <td>
                      <span>
                       <a href="/datamatakuliah_pertambangan/{{$m->id_matakuliah_pertambangan}}/show" title="Show details" class="btn btn-sm btn-clean btn-icon btn-icon-md kt-demo-icon__preview" >
                        <i class="flaticon-visible"></i>

                        <a href="/datamatakuliah_pertambangan/{{$m->id_matakuliah_pertambangan}}/edit" title="Edit details" class="btn btn-sm btn-clean btn-icon btn-icon-md">              
                         <i class="la la-edit"></i>

                         <a href="/datamatakuliah_pertambangan/{{$m->id_matakuliah_pertambangan}}/delete" title="Delete" class="btn btn-sm btn-clean btn-icon btn-icon-md">
       ...
       <!-- Modal -->
       <div class="modal fade" id="addMatakuliah_pertambangan" tabindex="-1" role="dialog" aria-labelledby="addMatakuliah_pertambanganTitle" aria-hidden="true">
         <div class="modal-dialog modal-dialog-scrollable" role="document">
           <div class="modal-content">
             <div class="modal-header">
               <h5 class="modal-title" id="addMatakuliah_pertambanganlableTitle">Tambah Data Matakuliah Pertambangan</h5>
               <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                 <span aria-hidden="true">&times;</span>
               </button>
             </div>
             <div class="modal-body">
              <form action="/datamatakuliah_pertambangan/create" method="POST">
                {{csrf_field()}}
                <div class="form-group">
                  <div class="form-group {!! $errors->has('kd_matakuliah_pertambangan') ? 'has-error' : '' !!}">
                    <label for="kd_matakuliah_pertambangan">Kode Matakuliah</label>
                    <input name="kd_matakuliah_pertambangan" type="text" class="form-control" value="{{ $nomatakuliah_pertambangan }}">
                    {!! $errors->first('kd_matakuliah_pertambangan', '<p class="help-block">:message</p>') !!}
                  </div>

                  <label for="exampleInputEmail1">Nama Matakuliah</label>
                  <input name="namamatakuliah_pertambangan" type="text" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Nama Matakuliah" required>
         ...
         <div id="detailMatakuliah_pertambangan" class="modal fade" tabindex="-1" aria-hidden="true" style="display: none;">
