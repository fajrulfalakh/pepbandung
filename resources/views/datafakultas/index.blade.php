...
<head>
  ...
  <!--end::Base Path -->
  <meta charset="utf-8" />
  <title>PEP BANDUNG | Fakultas</title>
    ...
    <div class="kt-grid__item kt-grid__item--fluid kt-grid kt-grid--hor kt-wrapper" id="kt_wrapper">
        ...
        <div class="kt-subheader   kt-grid__item" id="kt_subheader">
          <div class="kt-subheader__main" style="justify-content: flex-end;">
            <h3 class="kt-subheader__title"> Fakultas</h3>
            <span class="kt-subheader__separator kt-hidden"></span>
            <div class="kt-subheader__breadcrumbs">
              <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#addFakultas">Tambah Data
              </button>
              <!-- <span class="kt-subheader__breadcrumbs-link kt-subheader__breadcrumbs-link--active">Active link</span> -->
          </div>
          <div class="kt-portlet__head-toolbar">
              <div class="kt-portlet__head-wrapper">
                <a href="/datafakultas" class="btn btn-clean btn-icon-sm">
        ...
        <div class="portlet-body table-scrollable">
            <table class="table table-striped table-hover table-bordered table-sm jquery-datatables" cellspacing="0" width="100%">
             ...
             <tbody>
                ...
                @foreach ($dataFakultas as $m)
                <tr>
                   <td>{{ $m->id_fakultas }}</td>
                   <td>{{ $m->namafakultas }}</td>
                   <td>
                      <span>
                         <a href="/datafakultas/{{$m->id_fakultas}}/show" title="Show details" class="btn btn-sm btn-clean btn-icon btn-icon-md kt-demo-icon__preview" >
                            <i class="flaticon-visible"></i>
                            
                            <a href="/datafakultas/{{$m->id_fakultas}}/edit" title="Edit details" class="btn btn-sm btn-clean btn-icon btn-icon-md">              
                              <i class="la la-edit"></i>
                              
                              <a href="/datafakultas/{{$m->id_fakultas}}/delete" title="Delete" class="btn btn-sm btn-clean btn-icon btn-icon-md">
...
<!-- Modal -->
<div class="modal fade" id="addFakultas" tabindex="-1" role="dialog" aria-labelledby="addFakultasTitle" aria-hidden="true">
   <div class="modal-dialog modal-dialog-scrollable" role="document">
      <div class="modal-content">
         <div class="modal-header">
            <h5 class="modal-title" id="addFakultaslableTitle">Tambah Data Fakultas</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
               <span aria-hidden="true">&times;</span>
           </button>
       </div>
       <div class="modal-body">
          <form action="/datafakultas/create" method="POST">
             {{csrf_field()}}
             <label for="exampleInputEmail1">Nama fakultas</label>
             <input name="namafakultas" type="text" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Nama fakultas" required>
         </div>
